...
import { fetchPoolStats, formatUsdc, TIERS } from '../lib/aleo';
import {
...
const MOCK_CHART = [
  { t: 'D1', liquidity: 2.1, borrowed: 0.4 },
  { t: 'D2', liquidity: 2.8, borrowed: 0.9 },
  { t: 'D3', liquidity: 3.4, borrowed: 1.2 },
  { t: 'D4', liquidity: 4.1, borrowed: 1.8 },
  { t: 'D5', liquidity: 5.2, borrowed: 2.3 },
  { t: 'D6', liquidity: 6.8, borrowed: 3.1 },
  { t: 'Now', liquidity: 8.5, borrowed: 4.2 },
];

export default function DashboardPage() {
  const { poolStats, setPoolStats, wallet, creditScore, creditTier } = useStore();
    ...
    <div className="p-6 space-y-8 max-w-7xl mx-auto">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-zero-text-dim text-lg max-w-xl">
            Borrow without collateral. Prove your creditworthiness with
            zero-knowledge proofs — your data stays yours.
          </p>
          <div className="flex items-center gap-4 mt-6">
            {!wallet.connected ? (
              <Link href="/credit" className="btn-primary">
                Get Started →
              </Link>
            ) : creditScore ? (
            ...
            <Link href="/lend" className="btn-ghost">
              Earn Yield
            </Link>
          </div>
        </div>
        ...
        {[
          {
            ...
            value: poolStats ? formatUsdc(poolStats.totalLiquidity) : '—',
            icon: TrendingUp,
            color: '#00d4ff',
            change: '+12.4%',
            up: true,
          },
          {
            ...
            value: poolStats ? formatUsdc(poolStats.totalBorrowed) : '—',
            icon: ArrowUpRight,
            color: '#a855f7',
            change: '+8.1%',
            up: true,
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-4 text-xs text-zero-text-dim">
              <span className="flex items-center gap-1.5">
                <span className="w-3 h-0.5 bg-zero-cyan rounded inline-block" />
                Liquidity
              </span>
              <span className="flex items-center gap-1.5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={MOCK_CHART}>
              ...
              <YAxis tick={{ fill: '#6b7fa3', fontSize: 12 }} axisLine={false} tickLine={false} />
              <Tooltip
                contentStyle={{
                  background: '#0c1424',
        ...
        <div className="glass rounded-2xl p-6">
          <h3
            className="text-base font-semibold text-zero-text mb-6"
            style={{ fontFamily: "'Syne', sans-serif" }}
          >
            Borrower Tiers
          </h3>
        ...
        {[
          {
            ...
            desc: 'Your token amounts and loan sizes are never visible on-chain.',
            color: '#00d4ff',
          },
          {
            icon: Shield,
            title: 'ZK Credit Proofs',
            desc: 'Prove your creditworthiness without revealing any personal data.',
            color: '#a855f7',
          },
          {
            icon: Zap,
            title: 'Zero MEV',
            desc: 'Private order sizes eliminate front-running and MEV attacks.',
            color: '#00ffcc',
          },
          ...
          <div key={title} className="glass glass-hover rounded-2xl p-5">
            ...
            <h4
              className="text-sm font-semibold text-zero-text mb-2"
              style={{ fontFamily: "'Syne', sans-serif" }}
            >
              {title}
            </h4>
